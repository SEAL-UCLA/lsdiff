...
public class LSDLiteral {
	...
	public String toString() {
		...
		for (int i = 0; i < bindings.size(); i++) {
			...
			bs = bs + bindings.get(i).toString();

		}
		return (nonNegated ? "" : "!") + this.predicate.getDisplayName() + "(" + bs
				+ ")";
	}
	...
	public LSDLiteral substitute(LSDVariable toReplace, LSDBinding replacement)
		...
		if (freeVariables)
			...
			for (LSDBinding binding : newbs) {
				...
				bs = bs + binding.toString();

			}
			return LSDFact.createLSDFact(predicate, bs, this.nonNegated);
		}
	}
